...
-- Не матеарилизованное / временое представление - при запросе к представлению, например в блок FROM или JOIN или еще где, вместо результата представления будет подставляться и каждый раз заново исполняться запрос из представления
...
--                               Обновляемые представления. Вставка и удаление данных
...
ALTER VIEW old_view_name RENAME TO new_view_name;  -- Синтаксис переименования самого представления
-- можно удалять представления
DROP VIEW IF EXISTS view_name;  -- Синтаксис удаления представления (IF EXISTS не обязательно)
