...
-- DELETE  -  оператор удаления записей(строк) из таблицы. Является наиболее универсальным и безопасным. Записи удаляются по одной. Оставляет логи.
DELETE FROM имя_таблицы WHERE условие_отбора_записей;
-- Если условие отбора записей WHERE отсутствует, то будут поочередно удалены все записи указанной таблицы. Оптимизатор запросов СУБД MySQL автоматически использует оператор TRUNCATE, если оператор DELETE не содержит условия WHERE или конструкции LIMIT.
...
-- TRUNCATE  -  оператор удаления записей(строк) из таблицы
...
-- Удаляет все строки в таблице, не записывая при этом данные о удалении отдельных строк в журнал транзакций(логи)
-- Не может удалить данные, на которые есть ссылки из других таблиц
-- [MySQL] Сбрасывает счётчик идентификаторов до начального значения
