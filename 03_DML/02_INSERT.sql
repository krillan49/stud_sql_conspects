...
-- INSERT - добавляет/вставляет в таблицу новые данные/строки.
...
-- Можно опустить имена столбцов, если заполняем их по порядку и вставляем в строку данные всех столбцов
...
-- Во многих СУБД введён механизм автоматической генерации первичного ключа.

-- [PostgreSQL] имеет GENERATED AS IDENTITY или псевдотипы SMALLSERIAL, SERIAL, BIGSERIAL. Если столбец задан со сгенерированной последовательностью и целочисленным значением, то оно будет увеличиваться при добавлении новой записи, те значения будут генерироваться автоматически, но тогда нужно прописывать имена колонок
INSERT INTO chair (chair_name, dean) VALUES ('name', 'dean');

-- [MySQL ?] При создании новой записи в качестве значения good_id достаточно передать NULL или 0
...
-- 1. Вставка при создании новой таблицы. Чтобы создать новую таблицу и вставить в нее результат SELECT-запроса, нужно просто добавить INTO table_name между раделами SELECT и FROM этого SELECT-запроса. При этом повторно добавить в нее из запроса с другим условием не получится
