...
-- Жмем Finish
-- Когда подключение создано, сверху слева в дереве будет значек постгрес с зеленой галочной(значит подключено), открываем и видим все БД, со схемами и всем прочим

-- Чтобы добавить новую БД, то правой кнопкой нажимаем на значек подключенной СУБД -> 'Create' -> 'Database', даем имя и нажимаем 'OK'

-- Если мы раскываем конкретную БД в дереве то видим подветки, одна из них это 'Schemas' - тут располагаются таблицы и все данные, если раскрыть схемы то там будут располагаться подветки отдельных схем
...
-- При вставе запроса в окно редактора если нажать контрл+шифт+эф то вставится в отформатированном виде
