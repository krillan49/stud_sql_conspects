...
-- SQLite — это библиотека на языке C, которая реализует небольшой, автономный, полнофункциональный механизм базы данных SQL. Компактная встраиваемая СУБД. Исходный код библиотеки передан в общественное достояние. Легковесна и может быть встроенной в отдельные приложения. Не расчитана на использование по сети и высокие нагрузки.
...
-- > pragma table_info(articles);   -  Посмотреть, какие стобцы и их типы и прочее существуют в таблице articles (?? хз это скуэль запрос или командп консоли скюлайт ??)
