...
-- WHERE (где) - условный оператор, выбирает/фильтрует только те строки которые соответсвуют условию/ограничению (в сравнении можно использовать = > < >= <= != <>)(<> это тоже знак "не равно")
...
SELECT * FROM Orders WHERE DeliveryTime < '2013-06-20';  -- выбрать строки, где дата в столбце "DeliveryTime" меньше 20.06.2013. (? При сравнении дат они должны быть прописаны в том формате что задан в локали ?)
...
-- ?? Потом мб перенести в конец чтобы все типы условий с нул там и лайк объединить в запросе ??

-- OR|AND|XOR (или|и|исключающее или): логические операторы позволяют объединять несколько условий.
-- XOR(XOR похоже нет в постгресс) - только одно из сравниваемых значений должно быть истинно, а другое должно быть ложно
...
-- LIKE (поиск) - условный оператор позволяет нам искать строки, соответствующие шаблогу(паттерн матчинг).
...
SELECT name FROM Users WHERE email NOT LIKE '%@gmail.%'; -- выбираем где значение колонки почта не содержит '@gmail.'
SELECT name FROM Users WHERE NOT (email LIKE '%@gmail.%'); -- выбираем где значение колонки почта не содержит '@gmail.'
...
WHERE TO_TSVECTOR(lang, name) @@ TO_TSQUERY('english', 'Some')   -- поиск сразу по 2м колонкам
