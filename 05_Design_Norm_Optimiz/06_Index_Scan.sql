...
-- Индекс - структура данных и соответсвенно объект базы данных, который можно создавать и удалять с жеского диска. Он позволяет искать значения без полного перебора строк и сокращать время поиска, тоесть ускоряет выборку данных из таблицы за счет дополнительных операций записи и использования дополнительного пространства на диске

-- Индекс применяется для оптимизация выборки небольшого числа записей(относительно общего количества записей содержащихся в таблице). От того сколько записей фильтрует запрос зависит будет ли использован индекс
