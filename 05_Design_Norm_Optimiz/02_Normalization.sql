...
-- Нормальные формы именуются как: 1NF(1НФ), 2NF, 3NF, 4NF, 5NF, 6NF

-- 3NF - общепринятая и наиболее распространенная форма нормализации данных в индустрии, дальше нее, в подаляющем большинстве случаев нормализацию не проводят, тк тратить слишком много времени на нормализацию БД нерационально, за это время бизнес уже может что-то изменить и придется много раз все переделывать

-- Основным способом удовлетворения требований нормальных форм является декомпозиция, тоесть рабитие одной большой таблицы на несколько маленьких, более однозначных таблиц
...
-- Помогает избегать лишних JOIN-ов, тоесть SELECT-запросы могут проходить быстрее, но будет требоваться больше места для избыточных данных
...
-- 3. Все столбцы являются обычными - тоесть содержит только значение которое надо получить, чтобы какие-то одельные ячейки не содержали например функции, хранимые процедуры, ссылки итд. Тоесть данные в каждой колонке должны быть 1го типа не только по собственно типу, но и по способу их использования

-- Например таблица, авторов и книг:
-- в ней 1я и 3я строки дубликаты
-- в колонке books не простой тип данных, а массив и значения не скалярные, тк массив содержит разные книги во 2й строке:
