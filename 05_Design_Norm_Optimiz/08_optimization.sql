...
-- Буферный кэш - есть в PostgreSQL, ранее прочитанные страницы кэшируются и потом значения могут ситаться из него. При повторном выполнении запроса - обращается к буферному кэшу, не обращаясь к жескому диску, это ускоряем дальнейшие запросы.

-- Если места в буферном кзше не осталось, то удаляет оттуда страницу к которой меньше всего обращаемся

-- Это стоит учитывать при оптимизации, например если хотим чтобы в кэш записывался редкий запрос
...
-- Мы сначала по индексу нашли айдишники 39 фильмов(bitmap index scan) и это заняло всего 0.028..0.029 милисекунды и далее мы с этими 39 индексами обратились к таблице film (Bitmap Heap Scan on film) и из нее запросили только нужные нам 39 строк, а не все и это заняло 0.041..0.080 милисекунды. Итого общее время выполнения составило 0.146 ms
