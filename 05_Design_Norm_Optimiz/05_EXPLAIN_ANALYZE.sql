...
-- EXPLAIN - команда и аналитический инструмент, выводит план выполнения запроса (оценку планировщика) без выполнения самого запроса, где будет информация, каким образом запрос выполняется, виды сканирования по которым он производится. Так же выдаст инфу про то, какие cost(цены) предположительно возникают. Так как сам запрос реально выполняться не будет, оценка не займет много времени. Оценка планировщика не всегда может достаточно точно отражать реальность, а где-то и совсем не точно
