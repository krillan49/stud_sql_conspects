...
(
  ...
  -- COLLATE "C" - таблица не будет использовать колэйшен, тоесть правила сортировки и сравнения, если они по умолчанию привязаны например к кирилице, а будет использоваться просто побайтовое сравнение символов в строке, тк будем использовать латиницу для этих строк
...
-- Заполним колонку annotation отдельно, чтоб не генерило одинаковый текст:
UPDATE perf_test
...
-- Данный запрос будет исполняться примерно 2 секунды, что достаточно долго:
SELECT * FROM perf_test WHERE id = 3700000;

-- Проверим при помощи EXPLAIN(или EXPLAIN ANALYZE) почему он исполнялся так долго:
EXPLAIN SELECT * FROM perf_test WHERE id = 3700000;  --> 'Paralel Seq Scan', тоесть чтение выполняется при помощи самого долгого последовательного сканирования
...
EXPLAIN SELECT * FROM perf_test WHERE id = 3700000;  --> 'Bitmap Index Scan', тоесть теперь производится побитовое индексное сканирование

-- Теперь тот же запрос будет испольняться примерно 60 милисекунд(0.06 секунды):
SELECT * FROM perf_test WHERE id = 3700000;
...
EXPLAIN SELECT * FROM perf_test WHERE reason LIKE 'bc%' AND annotation LIKE 'AB%'; --> 'Paralel Seq Scan'
...
-- Проверим еще раз:
EXPLAIN SELECT * FROM perf_test WHERE reason LIKE 'bc%' AND annotation LIKE 'AB%'; --> 'Index Scan'

-- Теперь тот же запрос занимает 200 милисекунд
SELECT * FROM perf_test WHERE reason LIKE 'bc%' AND annotation LIKE 'AB%';


-- Построение индекса по 2м колонкам так же дает возможность индексного поиска по первой из них отдельно, но не дает отдельного поиска по второй, потому если он нужен, то придется сделать отдельный индекс
...
-- Чтобы заработало нужно создать отдельный индекс конкретно для использования с этой функцией (индекс по выражению)
...
SELECT * FROM perf_test WHERE reason LIKE '%bc%';         --> 3.3 секунды
