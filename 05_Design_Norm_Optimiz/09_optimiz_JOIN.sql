...
-- Видим количество строк и стоимости в результате каждого шага, например на каком-то шаге идет резкий рост строк или стоимости и мы можем попробовать этого как-то избежать
...
-- Но это не рекомендуется, тк планировщик запросов разумно оценивает сложность запроса, доступные ресурсы и статистику, чтобы определить подходящий способ соединения. Вместо того, чтобы вручную применять определенный алгоритм соединения, рассмотрите возможность оптимизации запросов, обеспечения наличия соответствующих индексов
