...
-- Возможное количество моделей, которые мы могли бы создать довольно большое, соответсвенно есть много разных подходов. Но в настоящее время, на реальных проектах проектированием БД не занимаются очень углубленно. Наболее важно не ошибиться в базовых вещах, потому на этап проектирование не отводится очень много времени, особенно в современных методологиях типа Аgile/Эджайл(Гибкие методологии разработки - agile software development). Индустрия во многом пришла к тому, что проще работать с предметной областью по ходу разработки и по ходу дела изменять проектные модели, тк начинаем лучше понимать как работает эта предметная область
...
-- Колличество стадий в резных теориях может быть разным - больше, чем описано тут
...
-- b. Разные источники включают разные компоненты в логическую модель - тоесть существует какое-то количество различных мнений о том как должна выглядеть логическая модель
-- c. Описание всех ключей во всех таблицах
-- d. Определение всех типов данных для всех свойств сущьностей. В теории это происходит безотносительно конкретной СУБД, но в реальности часто под конкретную СУБД
-- e. Описание всех логических ограничений - это спорный пункт, тк сразу описать все логические ограничения очень сложно, тк предметная область познается по ходу проектирования и потому какие-то логические ограничания возникают в процессе разработки
-- f. Нормализация отношений между таблицами, чаще всего до формы 3НФ, тк чаще всего более высокие формы приводят к избыточной нормализации
...
-- Так же при проектировании БД не стоит забывать, что о конвенциях наименований, тоесть о том как мы будем именовать объекты и прочих стилистических особенностях нужно договориться на этапе проектирования или в конце него, до того как мы начали заниматься реализаций



--                                         Entry Relationship Diagrams

-- Не обязательно, особенно в методологии Эджайл
