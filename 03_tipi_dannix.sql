...
CHAR(X)	   -- Содержит текстовые строки. Длина фиксируемая по выбору(0 до 255), указывается в скобках при объявлении. Если длина строки меньше указанной, она дополняется правыми пробелами до указанной длины.
...
-- [PostgreSQL] есть CHAR, VARCHAR, TEXT с точно таким же поведением
...
-- [PostgreSQL] есть SMALLINT, INTEGER/INT, BIGINT
...
NUMERIC   -- Дают высокую точность нецелых расчетов, например для финансовых операций
...
-- PostgreSQL - поддерживает массовы, JSON, XML, специальные геометрические типы, так же позволяет создавать свои кастомные типы
