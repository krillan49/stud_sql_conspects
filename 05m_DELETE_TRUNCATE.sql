...
-- DELETE и TRUNCATE  -  операторы удаления записей(строк) из таблицы. Наиболее универсальным и безопасным является DELETE

DELETE FROM имя_таблицы WHERE условие_отбора_записей;
-- Если условие отбора записей WHERE отсутствует, то будут удалены все записи указанной таблицы. Записи удаляются по одной. Оставляет логи
...
-- Не срабатывают триггеры, в частности, триггер удаления, не оставляет логи
