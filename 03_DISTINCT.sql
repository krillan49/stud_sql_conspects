...
-- DISTINCT исполняется после всех остальных действий, те дубликаты исключаются из результата запроса, соответственно можно провести сортировку, чтобы исключить дубликаты в том порядке в котором необходимо
...
-- Можно выбирать уникальные значения не только по полям, но и по выражениям
...
-- DISTINCT ON - исключает дубликаты строк только по выбранным столбцам, а не по всем что описаты в поле SELECT
