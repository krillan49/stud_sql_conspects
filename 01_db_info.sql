...
-- Системы управленя базами данных/Data base menegment systems (СУБД/DBMS) - программные средства для управления данными: отвечает за поддержку языка БД(например SQL), осуществляет доступ к БД и данным, позволяет их создавать, менять и удалять, обеспечивает безопасность, оптимизацию и т.д.
...
-- Транзакция это когда несколько операций обязательно должны быть вместе сохранены или наоборот обязательно вместе не сохранены

-- ACID (формальные критерии транзакционных систем):
-- 1. Atomicity (атомарность) - транзакционная система гарантирует, что все операции, в рамках одной транзакции будут или полностью записаны в БД, или если во время трянзакции произошел сбой, то произойдет откат, который приведет БД в состояние как будто этой транзакции не было. Пример в банке нужно одновременно списать с одного счета и зачислить на другой счет.
-- 2. Consistency (согласованность) - система гарантирует выполнение тех ограничений и правил, которые мы на нее наложим
-- 3. Isolation (изолированность) - в рамках выполнения одной транзакции, другие транзакции не повлияют на результат ее работы. Тоесть транзакция может состоять из нескольких операций и если между ними другая дранзакция изменит данные, которые могут повлиять на согласованность данных мерду разными операциями в 1й транзикции - изолированность гарантирует, что такого не будет и транзакция не будет учитывать изменения в данных сделанных после начала этой транзакции. Самый строгий вариант изолированности - это выполнение всех транзакций только последовательно оду за другой, соотв следующие ждут своей очереди. Менее строний вариант изолированности, для более нагруженных систем - транзакции могут выполняться параллельно и даже в некоторых случаях влиять на результаты друг друга
