...
-- JOIN - оператор многотабличного запроса, позволяет делать выборку из нескольких таблиц, соединяя данные(строки) из них в одном запросе. Позволяют соединять строку из одной таблицы той строкой из другой по соответствю значений столбцов в них, используя например внешний ключ одной таблицы и первичный другой

-- При выполнении запроса, соединение таблиц происходит в самую первую очередь. В результате соединения будет сформирована результирующая виртуальная таблица и далее уже из этой результирующей таблицы будут браться все данные для остальных действий запроса, например фильтрация, даже если она по полям только из 1й таблицы.
...
-- SELF  - соединение таблицы с самой собой как буд-то это 2 одинаковых таблицы
...
-- Внутреннее соединение - находятся пары записей из двух таблиц, удовлетворяющие условию соединения, тем самым образуя записи новой таблицы, что будет содержать поля из первой и второй исходных таблиц. Например если в нашем условии указано равенство значений в полях A.good_id и B.id, то при внутреннем соединении в итоговой выборке окажутся только те записи, где эти значения равны
...
-- 3. Далее берем уже 2ю, 3ю итд строки 1й таблицы и проверяем со всем строками 2й таблицы
...
-- Вывод: для оптимизации запроса 1м делом лучше соединить таблицы с наименьшим числом строк
...
-- INNER JOIN + псевдонимы + WHERE(осуществляется после объединения). Если используем * для вывода в запрсе от соединения нескольких таблиц то выведет все поля всех объединенных таблиц:
SELECT * FROM Customers C JOIN Orders O ON O.CustId = C.Id WHERE O.Total > 200;   -- соединяем те строки таблиц Customers и Orders, в которых O.CustId = C.Id
...
SELECT Res.room_id, AVG(Rev.rating) AS score
FROM Res JOIN Rev ON Res.id = Rev.res_id
...
-- Объединение по неравенству значений в колонке, тоесть соединяются значения тех строк таблиц, значения в выбранной колонке в которых не равны
...
-- Доп условия вроде AND r.count > 5 удобны для внешних соединений, чтобы неподходящие строки оставались со значением NULL а не фильтровались как при обычном условии в WHERE. И можно например сделать COUNT(*) и COUNT(some) в зпросе, чтобы вывести и общее число и число соответсвующих значений, тоесть не являющихся NULL
...
--  Работает как INNER JOIN, а соединение происходит автоматически по всем столбцам, что имеют одинаковые имена в обеих таблицах, тоесть еще короче чем с USING.
...
-- 1. LEFT OUTER JOIN (внешнее левое соединение) - возвращает все значения из левой таблицы, соединённые с соответствующими значениями из правой таблицы, там где они удовлетворяют условию соединения, а там где не удовлетворяют, будут значения из левой таблицы соединенные с значениями NULL, в полях правой таблицы. Тоесть все строки левой таблицы гарантировано попадут в результирующий набор.
...
-- 2. RIGHT OUTER JOIN (внешнее правое соединение): тоже что и левое только все значения возвращаются из правой, а из левой только соотв условию либо NULL.


-- 3. FULL OUTER JOIN (внешнее полное соединение). Реализовано не во всех СУБД.(Есть: PostgreSQL; Нет: MySQL)
...
-- SELF JOIN - используется тогда, когда нужно построить иерархию в запросе и соединить данные таблицы с другими данными той же самой таблицы. Это обработка одной и той же таблицы как двух отдельных таблиц, что позволяет связывать данные в таблице с другими данными в той же таблице.
...
-- У каждого работника может быть менеджер, но сам менеджер тоже работник и соответственно тоже может иметь менеджера, тоесть в таблице employee_id сотрудника может ссыласться на manager_id другого сотрудника
...
-- ...       |  ...
----------------------
-- Соединим эту таблицу саму с собой и в дополнительном условии укажем не просто неравенство id актеров, а один меньше другого, чтобы исключить все повторы и соотв улучшить производительность запроса
...
-- Элемент LATERAL может отображаться на верхнем уровне списка FROM или внутри JOIN дерева. В последнем случае он также может ссылаться на любые элементы, которые находятся слева от того JOIN в котором он находится.
