...
-- Для обеспечения собственной поддержки типов данных JSON в среде SQL - PostgreSQL реализует модель данных SQL/JSON. Эта модель состоит из последовательностей элементов. Каждый элемент может содержать скалярные значения SQL с дополнительным значением SQL/JSON null и составные структуры данных, которые используют массивы и объекты JSON. Модель является формализацией подразумеваемой модели данных в спецификации JSON RFC 7159

-- SQL/JSON позволяет обрабатывать данные JSON наряду с обычными данными SQL с поддержкой транзакций, включая:
...
-- Операторы извлечения поля/элемента/пути возвращают NULL, а не приводят к ошибке, если входные данные JSON не имеют правильной структуры для соответствия запросу; например, если такого ключа или элемента массива не существует.

-- Есть два способа получить данные для ключей в столбцах JSON. По индексу и по имени ключа. Они применимы как к столбцам json, так и к jsonb:
...
FROM sales
-- Запрос получает первый и второй элементы из массива после извлечения их из поля product и преобразует второй элемент к типу float
