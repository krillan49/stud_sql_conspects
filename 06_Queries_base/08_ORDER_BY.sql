...
-- Если не задавать сортировку явно, то нельзя быть на 100% уверенным, что записи будут выданы отсортированными по айди

-- ORDER BY - позволяет вывести строки по определенному(по умолчанию ASC) порядку, значений некого столбца, столбцов или выражения. Можно сортировать по колонкам таблиц, которые не выводятся данным запросом
...
-- Cортируем по нескольким столбцам: при равенстве значений в 1м столбце, сортирует по 2му итд
