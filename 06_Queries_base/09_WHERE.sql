...
-- WHERE  - условный оператор, выбирает только те строки которые соответсвуют условию, тоесть возвращают TRUE
-- Условие может вернуть значение TRUE, FALSE или NULL
-- Можно использовать операторы сравнения: =, >, <, >=, <=, !=, <> (<> алиас к !=)
...
-- Так же можно использовать математические операторы или функции в условии
...
SELECT * FROM dogs WHERE name LIKE 'A%' OR id < 10;              -- выбираем строки где значение столбца name начинается с A или id меньше 10
SELECT * FROM Orders WHERE Total > 90 AND OrTime > '2012-10-12'; -- выбираем строки где значение одного столбца больше 100 и другого больше даты заданной строкой
...
SELECT * FROM students WHERE NOT tuition_received;  -- в столбце tuition_received должно быть false(именно false а не null)
...
-- IS NULL - сравнивает проверяемое значение с NULL (some = NULL - не работает)
...
-- SIMILAR TO - похоже LIKE только дополнительно может выбирать любую букву из заданных в скобках через пайпы.
...
-- Можно производить поиск сразу по нескольким колонкам, записав их в одном операторе, в том же порядке с проверяемыми значениями
...
-- (?? Хз зачем звездочка)
SELECT * FROM sоme WHERE description ~* 'confetti|glitter|golden toilet|massage chair|video game|karaoke'
