...
-- 2. * - оператор выбирает все столбцы из таблицы, можно вывести все столбцы несколько раз
...
SELECT *, *, * FROM сars;  -- 3 раза выбрать все строки всех столбцов из таблицы "Cars"
...
-- 4. Можно добавлять новые столбцы с одинаковыми литералами во всех строках (или разными, тогда нужен условный оператор)
SELECT *, 'US' AS location FROM ussales;
SELECT *, CASE WHEN name = 'Vasya' THEN 'RU' ELSE 'US' END AS location FROM ussales;
