...
-- 3. WHERE - фильтрация, поэтому если в предыдущем пункте соединяем огромные таблицы, то лучше отфильтровать их до того в отдельном общем табличном выражении
...
-- 7. SELECT DISTINCT - оставляем только уникальные расчитанные строки (мб после сортировки ??)
