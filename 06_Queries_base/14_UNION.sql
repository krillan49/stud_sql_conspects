...
-- UNION     - объединенте запросов как множеств строк (объединение множеств)
-- INTERSECT - ищет общие строки у нескольких запросов (пересечение множеств)
-- EXCEPT    - возвращает записи одного запроса, которых нет в другом (разница множеств)


--                                                UNION [ALL]

-- UNION [ALL] - объединяет результаты запросов в одну результирующую таблицу.
...
-- UNION ALL не убирает дубликаты (не уберет дубликаты даже если добавить дистинкт в каждый запрос ? всмысле уберет в каждом запросе, но не уберет убликаты полученные при объединении ?)
...
-- INTERSECT (Пересечение запросов) Комбинирует два запроса SELECT, но возвращает только совпадающие записи первого и второго запрпоса SELECT
