...
WITH RECURSIVE result AS (
  ...
  -- Столбцы рекурсивной части должны соответсвовать столбцам стартовой части и иметь ту же последовательность написания
)
SELECT * FROM result; -->
--  i  | factorial
-- ----+-----------
...
-- Пример с циклической связью(тоесть сотрудник может быть менеджером сам себе прямо или через цепочку менеджеров). Задача найти всех таких сотрудников и вывести их id и массив с цепочкой айдишников цикла, где их айди 1й и последний
WITH RECURSIVE r AS (
  ...
  UNION ALL
  SELECT r_id, e.id, e.manager_id, ARRAY_APPEND(arr, e.id) AS arr, -- добавляем в массив значение айди менеджера
    CASE WHEN e.id = r_id THEN 1 ELSE 0 END AS cycle  -- смотрим если айди этого сотрудника(менеджера к записи из предыдущей итерации) такое же как r_id тоесть как изначальное айди одного из проверяемых сотрудников, то меняем значение cycle на 1, что значит цикл найден
...
-- Используйте ключевое слово WITH один раз вверху. Если какие-либо из ваших общих табличных выражений (CTE) являются рекурсивными (rCTE), вам также нужно добавить ключевое слово RECURSIVE вверху один раз, даже если не все CTE являются рекурсивными:
...
-- Руководство:
--
-- Если RECURSIVE указано, то подзапрос может SELECT ссылаться на себя по имени.
--
-- Выделено жирным шрифтом мое. И, еще более проницательно:
--
-- Другим эффектом RECURSIVEявляется то, что WITHзапросы не обязательно должны быть упорядочены : запрос может ссылаться на другой запрос, который находится позже в списке. (Однако циклические ссылки или взаимная рекурсия не реализованы.) Без RECURSIVEзапросы WITHмогут ссылаться только на WITH запросы того же уровня, которые находятся раньше в WITHсписке.
--
-- Жирное выделение мое снова. Это значит, что порядок WITHпредложений не имеет смысла, когда RECURSIVEиспользуется ключевое слово.
--
-- Кстати, поскольку cte1и cte2в примере не упоминаются во внешнем коде SELECTи SELECTсами по себе являются простыми командами (без побочных эффектов), они никогда не выполняются (если только на них не ссылаются в cte3)

-- Пример где 1й подзапрос не рекурсивный используется во 2м рекурсивном подзапросе.
WITH RECURSIVE
t AS
...
r AS (
  -- В стартовом подзапросе берем таблицу(подзапрос выше) и выводим строки всех категорий с их items(количество итемов)
  SELECT category_id AS id, category_id, parent, items FROM t -- дублируем category_id
  UNION ALL
  -- Добавляем родителей всех строк из набора предыдущей итерации, но оставляем айди от предыдущей итерации(тоесть в итоге из самой первой), а все остальные колонки от родителя, тоесть получаем строку родителя с указателем на id строки в стартовом наборе
