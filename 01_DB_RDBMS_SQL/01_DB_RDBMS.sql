...
-- Хранение и запрос данных в виде документов, подобном JSON. Документ - это набор(дерево) атрибутов(ключей и значений). Значения могут быть любыми типами данных(в том числе и ссылки на другие документы)
...
-- Реляционныеот слова "relations" тк таблмцы имеют связи друг с другом
...
-- Реляционные от слова - отношения в реляционной алгебре. Отношения на практике это таблицы
...
-- Информация хранится в связанных друг с другом таблицах. Таблицы состоят из строк/записей и столбцов/атрибутов. Каждый столбец имеет определённый тип данных
...
-- Менее строгий вариант изолированности, для более нагруженных систем - транзакции могут выполняться параллельно и даже в некоторых случаях влиять на результаты друг друга
