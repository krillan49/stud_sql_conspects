...
-- setval - функция устанавливающая текущее или следующее сгенерированное значение, принимает имя последовательности, значение которое хотим установить и true(значение по умолчанию, можно не писать, устанавливает текущее значение) или false(устанавливает следующее сгенерированное значение)
...
-- 1. ALTER SEQUENCE - оператор для изменения последовательности
...
-- 2. DROP SEQUENCE - оператор для удаления последовательности
DROP SEQUENCE seq4;



--                                     SERIAL. Последовательности и создание таблиц

-- (! SERIAL - этот псевдотип данных устпрел и рекомендуется им пользоваться только на версиях PostgreSQL ниже 10й)
...
-- Так же базируется на SEQUENCE. Начиная с 10й версии PostgreSQL реколмендуется использовать этот более продвинутый способ создания поля с автоинкрементом. Он избегает множества ошибок возникающих при использовании синтаксиса с псевдотипом SERIAL
