...
-- Разница между FOREIGN KEY и просто связанным полем заключается в их назначении и свойствах:
-- 1.FOREIGN KEY (внешний ключ) связывает записи в одной таблице с записями в другой через общее поле, которое является первичным ключом в другой таблице. Он помогает сохранять согласованность базы данных путём обеспечения так называемой «ссылочной целостности»
-- 2. Просто связанное поле (просто поле без явного объявления как FOREIGN KEY) может быть создано в таблице и использоваться без явной декларации. При получении данных с помощью запроса JOIN такое поле всё равно рассматривается как FOREIGN KEY, хотя механизм базы данных не знает об этом отношении
-- Таким образом, FOREIGN KEY гарантирует, что значение из таблицы точно есть в той таблице, на которую идёт ссылка, в то время как просто связанное поле просто указывает на связь между таблицами
