...
log_min_messages -- параметр конфигурации сервера, описывает уровень сообщений которые будут записываться в логе сервера (WARNING и EXCEPTION - записываются по умолчанию)
...
--                                      Синтаксис отлова/обработки исключений
...
EXCEPTION
WHEN SQLSTATE '12882' THEN  -- начинаем блок перехвата исключения по коду ошибки, переходим в этот блок если исключение отловлено:
	...
	RETURN NULL;
-- Если мы знаем что может быть несколько разных ошибок то можем прописать несколько блоков WHEN:
WHEN OTHERS THEN -- тут при помощи OTHERS ловим все остальные возможные ошибки
