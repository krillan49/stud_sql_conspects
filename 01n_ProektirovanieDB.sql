...
-- Предметрая область или домен - это то с чем/кем мы работаем, например организация со своими правилами, набором товаров, сотрудниками итд
...
-- b. Аналитический процесс с участием stakeholders тоесть владельцев или экспертов от предметной области. Разработчики встречаются с владельцами бизнеса и экспертами домена, которые помогают разобраться в предметной области и вместе начинают строить USE CASES, концептуальную схему БД итд
...
-- e. Полностью описывает все логические ограничения - это спорный пункт, тк сразу описать все логические ограничения очень сложно, тк предметная область познается по ходу проектирования и потому какието логические ограничания возникают в процессе разработки
...
-- Таблица определяется по тому, какие там есть объекты, события и абстрации
-- Таблица состоит из колонок/полей, которые представляют собой свойства объекта
...
-- 3. Вычисляемое поле, которое не служит для повышения производительности БД. Например поле для зарплаты за все время. Тк если это не нужно для быстродействия, то вычислительные операции лучше проводить отдельно, а в БД хранить лишь необходимые для этого данные.
