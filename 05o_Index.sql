-- Потом отделить план выполнения запроса в отдельный фаил ??
...
-- (Взято из фаила CREATE_ALTER_DROP, сопоставить с тем что тут) Индексы. Создание и удаление.
...
-- Индекс - структура данных и объект/структура базы данных, который можно создавать и удалять с жеского диска. Он позволяет искать значения без полного перебора строк и сокращать время поиска, тоесть ускоряет выборку данных из таблицы за счет дополнительных операций записи и использования дополнительного пространства на диске

-- Тк индекс это дополнительная структура/объект то он накладывает на нас доп расходы - например при вставке/редактировании/удалении новых записей мы не только записываем значения в таблицу, но и для всех существующих для нее индексов добавляем айдишник этой новой строки и все эти индексы перестраиваем, тк айдишники там отсортированы по значению индексируемого поля, те чем больше индексов в таблице тем больше расходы.
...
-- B-tree (сбалансированное дерево - самый распространенный)
-- hash   (Хэш-индекс)
...
EXPLAIN table_name; -- так же можно применить ко всей таблице(если нет Клиента с деревом ??)
