...
-- UNION (Объединение результатов запросов) - объединяет результаты запросов в одну результирующую таблицу.
-- По умолчанию убирает повторения в результирующей таблице.
-- Не путайте операции объединения результатов запросов с операциями объединения таблиц. Для этого служит оператор JOIN.
-- Чтобы UNION корректно сработал нужно чтобы результирующие таблицы каждого из SQL запросов имели одинаковое число столбцов, с сопоставимыми типами данных(тоесть могут быть преобразованы друг в друга) и в той же самой последовательности.
...
-- Например нам нужны все топовые фильмы, но по 2м разным критериям. Добавим так же столбец, чтобы было понятно по какому критерию фильм попал в результат(так будет как будто UNION ALL, потому что у повторяющихся фильмов будет разное значение во 2м поле, тоесть это уже не дубликаты)
...
-- Этими операциями над множествами можно объединять более 2х запросов, в том числе и разными
SELECT country FROM customers
INTERSECT
SELECT country FROM suppliers
EXCEPT
